...
cap  = pcapy.open_live(loopback, 1024, 1, 2)

def next_packet():
    global cap, CLIENT_IP, file_exists
    while True:
        h, p = cap.next()
        ...
        if (len(p) >= 34):
            ip_header = unpack('!BBHHHBBHBBBBBBBB', p[14:34])
...
while True:
    (header, payload) = next_packet()

    if(header == 'ek'):
        nalekh = True
        header, payload = h1, p1
    else:
        h1, p1 = header, payload
    ...
    if (exists('server_end.lock') == True):
