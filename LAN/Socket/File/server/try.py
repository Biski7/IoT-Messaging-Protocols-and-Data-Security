...
while True:
    ...
    if (src_ip_str == CLIENT_IP or dst_ip_str == CLIENT_IP):
        ...
        epoch, microsecond = header.getts()
        total_seconds = epoch + (microsecond  * (10**(-6)))
        if count == 0:
        ...
        with open('log.csv', 'a') as csvf:
            ...
            writer.writerow([total_seconds, source_port, destionation_port, sequence_number, acknowledgement_number, offset, C_W_R, ECN_echo, Urgent, Ack, Push, Reset, Sync, Finish, window, checksum, pointer])


        if (exists('server_end.lock') == True and exists('file_to_receive.txt') and Ack == 1 and Finish == 0):
            diff_total_seconds = total_seconds - total_seconds_out - 2   
            with open('log.csv', 'a') as csvf:
                writer = csv.writer(csvf, dialect= csv.excel)
