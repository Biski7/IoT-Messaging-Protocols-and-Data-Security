...
# temp_time_g_loopback = 0
# temp_time_gl_loopback = 0
total_time_loopback = 0
...
while True:
    ...
    total_seconds_loopback = epoch_loopback + (microsecond_loopback  * (10**(-6)))
    # e_str_loopback, ms_str_loopback = str(epoch_loopback), str(millisecond_loopback)
    # ems_str_loopback = e_str_loopback+'.'+ms_str_loopback
    # ems_fl_loopback= float(ems_str_loopback)

    if count_loopback == 0:
        total_seconds_out = total_seconds_loopback


    # temp_time_gl_loopback = ems_fl_loopback
    ...
    with open('log_loopback.csv', 'a') as csvf:
        ...
        writer.writerow([total_seconds_loopback, source_port_loopback, destination_port_loopback, sequence_number_loopback, acknowledgement_number_loopback, offset_loopback, C_W_R_loopback, ECN_echo_loopback, Urgent_loopback, Ack_loopback, Push_loopback, Reset_loopback, Sync_loopback, Finish_loopback, window_loopback, checksum_loopback, pointer_loopback])

    if (exists('client_end_loopback.lock') == True and finack_counter_loopback == 2 and Ack_loopback == 1 and Finish_loopback == 0):
        diff_total_seconds = total_seconds_loopback - total_seconds_out - 1 
        with open('log_loopback.csv', 'a') as csvf:
            writer = csv.writer(csvf, dialect= csv.excel)
