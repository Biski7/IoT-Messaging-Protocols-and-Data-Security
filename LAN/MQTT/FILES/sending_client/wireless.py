...
while True:
    ...
    total_seconds_wireless = epoch_wireless + (microsecond_wireless  * (10**(-6)))
    # e_str_wireless, ms_str_wireless = str(epoch_wireless), str(millisecond_wireless)
    # ems_str_wireless = e_str_wireless+'.'+ms_str_wireless
    # ems_fl_wireless = float(ems_str_wireless)

    if count_wireless == 0:
        total_seconds_out = total_seconds_wireless
    ...
    with open('log_wireless.csv', 'a') as csvf:
        ...
        writer.writerow([total_seconds_wireless, source_port_wireless, destination_port_wireless, sequence_number_wireless, acknowledgement_number_wireless, offset_wireless, C_W_R_wireless, ECN_echo_wireless, Urgent_wireless, Ack_wireless, Push_wireless, Reset_wireless, Sync_wireless, Finish_wireless, window_wireless, checksum_wireless, pointer_wireless])

    if (exists('client_end_wireless.lock') == True and finack_counter_wireless == 2 and Ack_wireless == 1 and Finish_wireless == 0):
        diff_total_seconds = total_seconds_wireless - total_seconds_out - 1      
        with open('log_wireless.csv', 'a') as csvf:
            writer = csv.writer(csvf, dialect= csv.excel)
